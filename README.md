...
> `apk update && apk add ca-bundle` (25.12 и новее) или `opkg update && opkg
> install ca-bundle` (24.10 и старше) — либо используйте `curl -fsSL … | sh`.
...
**OpenWrt 25.12** — текущая стабильная ветка; сам демон в ней ничего не теряет
(fw4, netifd, procd, UCI и `kmod-tun` те же), но пакетный менеджер сменился с
`opkg` на `apk`. Поэтому пакет в релизах теперь `.apk`, собранный SDK 25.12, а
командам `opkg …` из старых инструкций на 25.12 соответствуют `apk …`. На 24.10
и старше ставьте установщиком: статический бинарник от версии ОС не зависит.
...
(`http://127.0.0.1:79/rci/show/version`) и откажется ставиться, если его нет.

**KeeneticOS 5.0 и 5.1** — используемые демоном parse-команды (`interface …
wireguard peer`, `ip global auto`, `ip route … auto`) в пятой ветке не менялись,
Entware и раскладка `/opt` тоже. Единственное, что её касается: в 5.1.2 RCI
отвечает 403 на запросы с `Host: localhost`, поэтому и демон, и установщик ходят
на `127.0.0.1` — IP-форму принимают все версии.

Ломающих изменений выше минимальных версий нет; «рекомендуется» — это то,
...
| Архитектуры | x86_64, aarch64, armv7, mipsel | mipsel (MT7621), aarch64 (MT7622/EN7581) |

Каждый пакет проходит userspace дважды — TURN/DTLS-прокси плюс wireguard-go, —
поэтому потолок скорости упирается в CPU роутера, а не в канал. Разумный минимум
— двухъядерный SoC уровня MT7621; заметно лучше aarch64 с ARMv8-крипторасширениями
(MT7622, MT7981, IPQ807x), где AES считается аппаратно. Замеров по конкретным
моделям нет.

Сборок под ARMv5 (kirkwood, at91, mxs) и big-endian MIPS (ath79) нет: это
железо 2015–2017 годов, одноядерное и без крипто-ускорения, на двойное
шифрование его не хватает. Установщик на таких роутерах откажется ставиться с
явным сообщением, а не подсунет несовместимый бинарник. Модели Keenetic на
big-endian MIPS не существуют — там отказ по той же причине.
...
Перед установкой скачанный файл проверяется: заголовок ELF (иначе это страница
404 или заглушка провайдера), совпадение архитектуры, контрольная сумма
`sha256` из релиза и наличие места на разделе. Бинарник кладётся временным
файлом и переименовывается, так что обновление поверх работающего демона
безопасно — сервис после подмены перезапускается сам.

Если платформа не определилась (кастомная сборка, Entware не на Keenetic,
выключенный веб-интерфейс прошивки), установщик не падает, а показывает причину
и предлагает выбрать платформу из списка. Вопрос задаётся через `/dev/tty`, так
что меню работает и при запуске через конвейер `| sh`.

Когда терминала нет (cron, CI, `docker` без `-t`) — задайте платформу явно:

```sh
wget -O - .../install.sh | WGKEYBOT_PLATFORM=openwrt sh   # или keenetic
```
...
В релизах есть `.apk` для aarch64_cortex-a53, mipsel_24kc и x86_64, собранный
SDK OpenWrt 25.12:

```sh
apk add --allow-untrusted ./wgkeybot-*.apk
```

`--allow-untrusted` нужен, потому что пакет не подписан ключом вашего feed'а.
Пакет ставит демон, procd-сервис и UCI-конфиг.

На 24.10 и старше (там ещё `opkg`, формат `.ipk`) пакета в релизах нет —
пользуйтесь установщиком, он кладёт то же самое.
...
- шесть статических бинарников — `wgkeybot-{amd64,arm64,armv7,mipsle}`
  ...
  рядом с каждым — `<ассет>.sha256`, который сверяет установщик;
- `.apk` для aarch64_cortex-a53 / mipsel_24kc / x86_64 через SDK OpenWrt 25.12;
