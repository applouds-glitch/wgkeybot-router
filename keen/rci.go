...
// Из Entware доступен на http://127.0.0.1:79/rci без авторизации. Адрес именно
// числовой: в 5.1.2 прошивка отвечает 403 на Host: localhost.
//
...
// Форматы ответов записаны по документации/сообществу для 4.x (в 5.x
// используемые команды не менялись); помечены
...
// NewRCI создаёт клиент. base — например "http://127.0.0.1:79/rci" (без
