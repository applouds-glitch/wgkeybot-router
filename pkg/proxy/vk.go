...
// time-based gap below spaces sequential requests from different slots.
...
func fetchVkCreds(ctx context.Context, link string) (string, string, []string, int, error) {
	client, err := tlsclient.NewHttpClient(
		...
		// migrate onto Russian roots (see vk_certs.go).
	...
	for i := 0; i < len(vkCredentialsList); i++ {
		...
		// held only around the gap calculation and the timestamp update
		// — getTokenChain itself runs unlocked, so a blocking browser
		// captcha in one goroutine never stalls another group's fetches.
		vkRequestMu.Lock()
		for !globalLastVkFetchTime.IsZero() {
			minGap := 3*time.Second + time.Duration(rand.Intn(3000))*time.Millisecond
			wait := time.Until(globalLastVkFetchTime.Add(minGap))
			if wait <= 0 {
				break
			}
			vkRequestMu.Unlock()
...
func getTokenChain(ctx context.Context, link string, creds VKCredentials, client tlsclient.HttpClient, profile Profile) (string, string, []string, int, error) {
	...
	enableManual := true
	// The host captcha model here is a single visible browser solve
	// (captcha_host.go: RequestCaptcha → host opens a browser), so the manual
	// step already IS the visible one. A second ManualVisible round would just
	// reopen the same browser. Keep it false. (Android splits the two: an
	// invisible WebView first, then a visible dialog.)
	...
	for attempt := 0; ; attempt++ {
		...
		if errObj, hasErr := resp["error"].(map[string]interface{}); hasErr {
			...
			} else if errCode, ok2 := errObj["error_code"].(float64); ok2 {
			...
			if captchaErr != nil && captchaErr.IsCaptchaError() {
				...
				solveMode, hasSolveMode := captchaSolveModeForAttempt(captchaSolveAttempt, enableManual, enableManualVisible, autoCaptchaSliderPOC)
				...
				if solveErr != nil {
					...
					captchaSolveAttempt++
					nextSolveMode, hasNextSolveMode := captchaSolveModeForAttempt(captchaSolveAttempt, enableManual, enableManualVisible, autoCaptchaSliderPOC)
					if hasNextSolveMode {
